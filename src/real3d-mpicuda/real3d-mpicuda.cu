...
int main(int argc, char **argv) {
   ...
   tmpxyz = alloc_double_tensor(localNx, Ny, Nz);
   ...
   pin_complex_tensor(psi, localNx, Ny, Nz);
   ...
   d_psi = alloc_complex_tensor_device(localNx, Ny, Nz);
   d_psi_t = alloc_complex_tensor_device(Nx, localNy, Nz);
   ...
   d_calphax = alloc_complex_vector_device(Nx - 1);
   d_calphay = alloc_complex_vector_device(Ny - 1);
   d_calphaz = alloc_complex_vector_device(Nz - 1);
   d_cbeta = make_cudaPitchedPtr(d_psi_t.ptr, d_psi_t.pitch, Nz, Ny);
   d_cbeta_t = make_cudaPitchedPtr(d_psi.ptr, d_psi.pitch, Nz, localNy);
   d_cgammax = alloc_complex_vector_device(Nx - 1);
   d_cgammay = alloc_complex_vector_device(Ny - 1);
   d_cgammaz = alloc_complex_vector_device(Nz - 1);
   ...
   } else dyna = fopen("/dev/null", "w");

   if (opt == 2) par = 2.;
   else par = 1.;

   fprintf(out, " Real time propagation 3D,   OPTION = %d, MPI_NUM_PROCS = %d\n\n", opt, nprocs);
   ...
   fprintf(out, " * Change for dynamics: GPAR = %.3f, GDPAR = %.3f *\n\n", gpar, gdpar);
   fprintf(out, "                  --------------------------------------------------------\n");
   ...
   psiparam.srcPtr = make_cudaPitchedPtr(psi[0][0], Nz * sizeof(cuDoubleComplex), Nz, Ny);
   psiparam.dstPtr = d_psi;
   psiparam.extent = make_cudaExtent(Nz * sizeof(cuDoubleComplex), Ny, localNx);
   psiparam.kind = cudaMemcpyHostToDevice;

   psiNx2Ny2Nz2param.srcPtr = make_cudaPitchedPtr((cuDoubleComplex *)((((char *)d_psi.ptr) + offsetNx2 * d_psi.pitch * Ny) + Ny2 * d_psi.pitch) + Nz2, sizeof(cuDoubleComplex), 1, 1);
   psiNx2Ny2Nz2param.dstPtr = make_cudaPitchedPtr(&psiNx2Ny2Nz2, sizeof(cuDoubleComplex), 1, 1);
   psiNx2Ny2Nz2param.extent = make_cudaExtent(sizeof(cuDoubleComplex), 1, 1);
   ...
   MPI_Bcast(&psiNx2Ny2Nz2, 2, MPI_DOUBLE, rankNx2, MPI_COMM_WORLD);
   fprintf(out, "Initial : %15.4f %11.5f %11.5f %10.5f %10.5f\n", norm, mu / par, en / par, *rms, cuCabs(psiNx2Ny2Nz2) * cuCabs(psiNx2Ny2Nz2));
   fflush(out);
   if (rmsout != NULL) {
   ...
   if (Nstp != 0) {
      ...
      for (cnti = 0; cnti < Nstp; cnti ++) {
         ...
         calcpsidd2(d_psi, d_psi_t, d_psidd2, d_psidd2_t, d_psidd2tmp);
   ...
   for (cnti = 0; cnti < Nrun; cnti ++) {
      ...
      if ((dynaout != NULL) && (cnti % outstpt == 0)) {
         calcrms(rms, d_psi, d_psi_t, d_tmpyzx, d_tmpxzy, d_tmpxyz, d_tmpyz, d_tmpxz, d_tmpxy, d_tmpx, d_tmpy);
         fprintf(dyna, "%8le   %8le   %8le   %8le   %8le\n",  (cnti * dt  + dt * Npas) * par, rms[0], rms[1], rms[2], rms[3]);
      }
   }
   iter_time += stop_cuda_timer(start, stop);
   ...
   free_pinned_complex_tensor(psi);
   ...
   free_complex_tensor_device(d_psi);
   free_complex_tensor_device(d_psi_t);
   ...
   free_complex_vector_device(d_calphax);
   free_complex_vector_device(d_calphay);
   free_complex_vector_device(d_calphaz);
   free_complex_vector_device(d_cgammax);
   free_complex_vector_device(d_cgammay);
   free_complex_vector_device(d_cgammaz);
...
void readpar() {
   ...
   if ((cfg_tmp = cfg_read("GDDPAR")) == NULL) {
      fprintf(stderr, "GDDPAR is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   gdpar = atof(cfg_tmp);
 ...
 */
void initpsi(cuDoubleComplex ***psi, double *tmpz) {
   long cnti, cntj, cntk;
   double cpsi;
   double tmp;
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               psi[cnti][cntj][cntk] = make_cuDoubleComplex(tmp / cpsi, 0.);
            }
         }
      }
   }
   ...
   if (Nstp == 0) {
      ...
      MPI_File_open(MPI_COMM_WORLD, "imag3d-den.bin", MPI_MODE_RDONLY, MPI_INFO_NULL, &file);
      if (file == MPI_FILE_NULL) {
         printf("Run the program using the input file to read. e.g.: imag3d-den.bin\n");
         MPI_Finalize(); exit(EXIT_FAILURE);
      }
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            ...
            for (cntk = 0; cntk < Nz; cntk ++) {
               psi[cnti][cntj][cntk] = make_cuDoubleComplex(sqrt(tmpz[cntk]), 0.);
            }
   ...
   for(r = 0; r < nprocs; r++) {
      ...
      if (r == rank) {
         for(cnti = 0; cnti < localNx; cnti ++) {
            for(cntj = 0; cntj < Ny; cntj ++) {
               for(cntk = 0; cntk < Nz; cntk ++) {
                  printf("[%02ld][%02ld][%02ld] = %1.15e\n", offsetNx + cnti, cntj, cntk, pot[cnti][cntj][cntk]);
               }
            }
         }
      }
...
void initdevice(cudaPitchedPtr psi, cudaPitchedPtr psi_t, cudaPitchedPtr dpsi, cudaPitchedPtr dpsi_t, cudaPitchedPtr psidd2, cudaPitchedPtr psidd2_t) {
   ...
   cudaCheckError(cudaMemcpy(d_calphax, calphax, (Nx - 1) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
   cudaCheckError(cudaMemcpy(d_calphay, calphay, (Ny - 1) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
   cudaCheckError(cudaMemcpy(d_calphaz, calphaz, (Nz - 1) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));

   cudaCheckError(cudaMemcpy(d_cgammax, cgammax, (Nx - 1) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
   cudaCheckError(cudaMemcpy(d_cgammay, cgammay, (Ny - 1) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
   cudaCheckError(cudaMemcpy(d_cgammaz, cgammaz, (Nz - 1) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
   ...
   tran_psi = init_transpose_complex(nprocs, localNx, localNy, Ny, Nz, psi.ptr, psi.pitch, psi_t.ptr, psi_t.pitch);
...
__global__ void calcmuen_kernel1x(cudaPitchedPtr psi, cudaPitchedPtr dpsi, double par) {
   long cnti, cntj, cntk;
   cuDoubleComplex *psirow1, *psirow2, *psirow3, *psirow4;
   double *dpsirow;
   double dpsix;
   ...
   for (cntj = TID_Z; cntj < d_localNy; cntj += GRID_STRIDE_Z) {
      for (cntk = TID_Y; cntk < d_Nz; cntk += GRID_STRIDE_Y) {
         for (cnti = TID_X; cnti < d_Nx; cnti += GRID_STRIDE_X) {
            ...
            if (cnti == 1) {
               psirow1 = get_complex_tensor_row(psi, 2, cntj);
               psirow2 = get_complex_tensor_row(psi, 0, cntj);

               dpsix = (cuCabs(psirow1[cntk]) - cuCabs(psirow2[cntk])) / (2. * d_dx);
            }

            if (cnti > 1 && cnti < d_Nx - 2) {
               psirow1 = get_complex_tensor_row(psi, cnti - 2, cntj);
               psirow2 = get_complex_tensor_row(psi, cnti - 1, cntj);
               psirow3 = get_complex_tensor_row(psi, cnti + 1, cntj);
               psirow4 = get_complex_tensor_row(psi, cnti + 2, cntj);

               dpsix = (cuCabs(psirow1[cntk]) - 8. * cuCabs(psirow2[cntk]) + 8. * cuCabs(psirow3[cntk]) - cuCabs(psirow4[cntk])) / (12. * d_dx);
            }

            if (cnti == d_Nx - 2) {
               psirow1 = get_complex_tensor_row(psi, d_Nx - 1, cntj);
               psirow2 = get_complex_tensor_row(psi, d_Nx - 3, cntj);
   ...
   for (cnti = TID_Z; cnti < d_localNx; cnti += GRID_STRIDE_Z) {
      for (cntk = TID_Y; cntk < d_Nz; cntk += GRID_STRIDE_Y) {
         for (cntj = TID_X; cntj < d_Ny; cntj += GRID_STRIDE_X) {
            ...
            if (cntj == 1) {
               psirow1 = get_complex_tensor_row(psi, cnti, 2);
               psirow2 = get_complex_tensor_row(psi, cnti, 0);

               dpsiy = (cuCabs(psirow1[cntk]) - cuCabs(psirow2[cntk])) / (2. * d_dy);
            }

            if (cntj > 1 && cntj < d_Ny - 2) {
               psirow1 = get_complex_tensor_row(psi, cnti, cntj - 2);
               psirow2 = get_complex_tensor_row(psi, cnti, cntj - 1);
               psirow3 = get_complex_tensor_row(psi, cnti, cntj + 1);
               psirow4 = get_complex_tensor_row(psi, cnti, cntj + 2);

               dpsiy = (cuCabs(psirow1[cntk]) - 8. * cuCabs(psirow2[cntk]) + 8. * cuCabs(psirow3[cntk]) - cuCabs(psirow4[cntk])) / (12. * d_dy);
            }

            if (cntj == d_Ny - 2) {
               psirow1 = get_complex_tensor_row(psi, cnti, d_Ny - 1);
               psirow2 = get_complex_tensor_row(psi, cnti, d_Ny - 3);
   ...
   for (cnti = TID_Z; cnti < d_localNx; cnti += GRID_STRIDE_Z) {
      for (cntj = TID_Y; cntj < d_Ny; cntj += GRID_STRIDE_Y) {
         ...
         for (cntk = TID_X; cntk < d_Nz; cntk += GRID_STRIDE_X) {
            ...
            if (cntk == 1) {
               dpsiz = (cuCabs(psirow[2]) - cuCabs(psirow[0])) / (2. * d_dz);
            }

            if (cntk > 1 && cntk < d_Nz - 2) {
               dpsiz = (cuCabs(psirow[cntk - 2]) - 8. * cuCabs(psirow[cntk - 1]) + 8. * cuCabs(psirow[cntk + 1]) - cuCabs(psirow[cntk + 2])) / (12. * d_dz);
            }

            if (cntk == d_Nz - 2) {
               dpsiz = (cuCabs(psirow[d_Nz - 1]) - cuCabs(psirow[d_Nz - 3])) / (2. * d_dz);
            }
...
__global__ void calcmuen_kernel2(cudaPitchedPtr psi, cudaPitchedPtr dpsi, cudaPitchedPtr psidd2, cudaPitchedPtr pot, cudaPitchedPtr tmpmu, cudaPitchedPtr tmpen, double g, double gd) {
   long cnti, cntj, cntk;
   cuDoubleComplex *psirow;
   double *psidd2row, *tmpmurow, *tmpenrow, *potrow, *dpsirow;
 ...
 *    psidd2tmp - temporary array holding first slice of FFT values
 */
 ...
 *    psi_t   - temporary array holding transposed wave function
 *    cbeta_t - Crank-Nicolson scheme coefficients
 */
void calclux(cudaPitchedPtr psi_t, cudaPitchedPtr cbeta_t) {

   transpose(tran_psi);

   calclux_kernel<<<dimGrid2d, dimBlock2d>>>(psi_t, cbeta_t, d_calphax, d_cgammax);
   cudaCheckError(cudaDeviceSynchronize());
...
void outdenx(cuDoubleComplex ***psi, double *tmpy, double *tmpz, FILE *file) {
   long cnti, cntj, cntk;

   for (cnti = 0; cnti < Nx; cnti += outstpx) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            tmpz[cntk] = cuCabs(psi[cnti][cntj][cntk]);
            tmpz[cntk] *= tmpz[cntk];
         }
...
__global__ void outpsi2xz_kernel(cudaPitchedPtr psi, cudaPitchedPtr psi2, long outstpx, long outstpz) {
   ...
   for (cnti = TID_Y * outstpx; cnti < d_localNx; cnti += GRID_STRIDE_Y) {
      psirow = get_complex_tensor_row(psi, cnti, d_Ny2);
      psi2row = get_double_matrix_row(psi2, cnti / outstpx);
...
__global__ void outpsi2yz_kernel(cudaPitchedPtr psi, cudaPitchedPtr psi2, long outstpy, long outstpz) {
   ...
   for (cntj = TID_Y * outstpy; cntj < d_Ny; cntj += GRID_STRIDE_Y) {
      psirow = get_complex_tensor_row(psi, offsetNx2, cntj);
